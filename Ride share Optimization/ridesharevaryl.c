...
void alg_tsg_train(struct task* tasks,int k,int l, int tsg)
{
   int i, t;
    double max_val;
    ...
    double val=0;
     int* founds;
    founds = (int*) malloc((n)*sizeof (int));
    ...
    {max_val=0;
               ...
               if (sz_tsg[t]==k)
                              {
                                  ...
                                  { S_temp=S_tsg;
                                      S_temp[t][h]=i;
                                      val=function_computation(tasks, t ,  S_temp[t] , k);
                                      if(val > max_valsi[t][i])
                                      {
                                          if(max_valsi[t][i]>function_computation(tasks, t ,  S_tsg[t] , k))
                                      {
                                          max_valsi[t][i]=val;
                                          best_rep[t][i]=h;
                                      }
                ...
                if (sz_tsg[t]<k)
                {
                    ...
                    S_temp[t][ind]=i;
                    if (sz_tsg[t]>0)
                    val = function_computation(tasks, t ,  S_temp[t] , ind+1);
                     if (sz_tsg[t]==0)
                         val = function_computation(tasks, t ,  S_temp[t] , ind+1);

                    max_valsi[t][i]=val;
                    best_rep[t][i]=sz_tsg[t];
                }
               
                valsind[i]=valsind[i]+max_valsi[t][i];
                
            }
                if(founds[i]==1)
                    valsind[i]=-1000;
           if(valsind[i] > max_val)
           {
               max_val=valsind[i];
               best_ind=i;
           }
        ...
        for(t=0;t<m;t++)
        {
            ...
            if (sz_tsg[t]<k)
            {
                S_tsg[t][best_rep[t][best_ind]]=best_ind;
                sz_tsg[t]++;
            }
            if (sz_tsg[t]==k)
            {
                if(max_valsi[t][best_ind]>function_computation(tasks, t ,  S_tsg[t] , k))
                        S_tsg[t][best_rep[t][best_ind]]=best_ind;
            }
    }
...
double test_perfomance_tsg(struct task* tasks, int k,int l,int tsg)
{
    int i, t, round, best_item, found, num_founds;
    double max_val,val,f_S_ind, test_val;
    ...
    for(i=0;i<tsg;i++)
    {
        for(t=0;t<m;t++)
        {
            S_final[t][0] = S_stage[i];
            val = function_computation(tasks, t ,  S_final[t] , 1);
            marginals_personal[t][S_stage[i]] = val;

            if(val > max_vals[t])
            {
                max_vals[t] = val;
                best_items[t] = S_stage[i];
            }
        }
    }
    for(t=0;t<m;t++)
       ...
       // printf("round = %d \n", round );
        ...
        while(found == 0)
        {
            ...
            for(i=0;i<tsg;i++)
            {
                ...
                for(t=0;t<m;t++)
                    if(founds[t] == 0)
                        if(marginals_personal[t][S_stage[i]] > max_vals[t])
                        {
                            max_vals[t] = marginals_personal[t][S_stage[i]];
                            best_items[t] = S_stage[i];
                        }
            }
            ...
            for(t=0;t<m;t++)
            {
                if(founds[t] == 0)
                {
                    ...
                    founds[t] = 1;
                    for(i=0;i<tsg;i++)
                        if(marginals_personal[t][best_items[t]] < marginals_personal[t][S_stage[i]])
                        {
                            founds[t]  = 0;
                            S_final[t][round] = S_stage[i];
                            
                            val = function_computation(tasks, t , S_final[t] , round + 1);

                            marginals_personal[t][S_stage[i]] = val - f_S[t];
                        }
...
//*******
   ...
   // exit(1);
    ...
    for(i=0;i<m;i++)
        ...
        sz_tsg = (int*) malloc((m)*sizeof (int));
        S_stage = (int*) malloc((tsg)*sizeof (int));

        best_rep = (int**) malloc((m)*sizeof (int*));
        for(i=0;i<m;i++)
              best_rep[i] = (int*) malloc((n)*sizeof (int));
        S_temp = (int**) malloc((m)*sizeof (int*));
        for(i=0;i<m;i++)
                     S_temp[i] = (int*) malloc((n)*sizeof (int));
        S_tsg = (int**) malloc((m)*sizeof (int*));
               for(i=0;i<m;i++)
                            S_tsg[i] = (int*) malloc((n)*sizeof (int));
        valsind = (double*) malloc((n)*sizeof (double));
        max_valsi = (double**) malloc((m)*sizeof (double*));
        for(i=0;i<m;i++)
                            max_valsi[i] = (double*) malloc((n)*sizeof (double));
   ...
   alg_rand_lazy_version(tasks,k,l);
     //   printf("Algorithm two-stage ... \n");
   //     alg_tsg_train(tasks,k,l,tsg);
    ...
    printf("upper bound on test performance (full greedy for each task)= %lf, k=%d\n", result[ind_k][5],k);
     ...
     //   result[ind_k][6]=test_perfomance_tsg(new_tasks, k,l,tsg)/points_per_task;
     //   printf(" test performance (two-stage)= %lf, k=%d\n", result[ind_k][6],k);


    } printf("[");
    for(int ind_k=0;ind_k<max_ind_k;ind_k++)
   // printf("%f %f %f %f %f %f %f;\n", result[ind_k][0],result[ind_k][1],result[ind_k][2],result[ind_k][3],result[ind_k][4],result[ind_k][5],result[ind_k][6]);
